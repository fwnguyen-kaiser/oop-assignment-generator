...
<p>
  ...
  <img alt="Tests" src="https://img.shields.io/badge/tests-153%20passing-2EA44F?style=flat-square&logo=pytest&logoColor=white">
...
| 6 | Phase 2 output under fuzz | **L4** | 🟡 **L4 forever** (sampling an infinite graph space is L4 at trial 50 and at trial 500,000) — but now against the **real oracle**: `tests/test_repair_fuzz_javac.py` round-trips 50 generated graphs through the production path (`repair` → `compile_logical_plan` → `build_ast` → `render_class`) into real `javac`, including interfaces and a `max_classes=3` cap that actually fires rule `2.5`. Verified non-vacuous: 40 probe trials rendered 109 classes, 12 with real `interface` declarations, 27 with an entity dropped. The 3 older `hypothesis` tests still assert pure-Python invariants only |
...
- **153 automated tests pass** (`pytest tests/ -q`), covering all 7 phases, including a reproduction of every real bug found — not just happy paths.
...
tests/                             153 tests, including reproductions of every real bug found

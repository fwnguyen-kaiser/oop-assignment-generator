...
- **Live end-to-end run on the current tree** (`e_commerce` × `advanced`, real Gemini + real JDK), re-done after the config-convention pass so the evidence matches the shipped configs: exit code 0, Phase 6 compiled on the first attempt with no deterministic repair needed, `[SKELETON_GATE] Student skeleton compiles cleanly`, all 9 conformance requirements satisfied, and all three output directories re-verified independently with `javac` (exit 0 each). `conformance_log.jsonl` was appended; `novel_shapes_log.jsonl` was correctly *not* created, since no unrecognised `javac` shape occurred. The config fix is confirmed live: repair took **no action at all** this run (every lossiness counter zero, down from `retype: 2` before the fix), and `aggregation` moved from an optional check that was being silently retyped to a **required** one that the design actually delivered — two surviving aggregation edges (`ShoppingCart → Product`, `Order → Product`). `conformance_log.jsonl` now holds both runs; `novel_shapes_log.jsonl` is still absent, so the row-9 baseline stands at zero novel `javac` shapes across two runs. Two things this run did *not* exercise, stated rather than implied: the failure branches (conformance exit 1, skeleton-gate refusal, novel-shape logging), which remain unit-tested only; and `e_commerce.yaml`'s corrected `PhysicalProduct → Taxable` hint, because the LLM chose a different interface entirely (`CreditCardPayment implements Payment`) — which is semantic-first working as designed, the hint is guidance and not an instruction.

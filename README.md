...
- **Live end-to-end run** (`e_commerce` × `advanced`, real Gemini + real JDK) — covering the gate/reporting work, but taken *before* the config-convention pass that followed it, so the config changes are backed by the offline 15-combination matrix test rather than by this run: exit code 0, Phase 6 compiled on the first attempt with no deterministic repair needed, `[SKELETON_GATE] Student skeleton compiles cleanly`, all 9 conformance requirements satisfied, and all three output directories re-verified independently with `javac` (exit 0 each). `conformance_log.jsonl` was appended; `novel_shapes_log.jsonl` was correctly *not* created, since no unrecognised `javac` shape occurred. The failure branches (conformance exit 1, skeleton-gate refusal, novel-shape logging) are covered by unit tests but have not yet fired in a live run, and the current config set has not been through a live run at all — both are honest gaps in *live* evidence, not in verification: everything here is covered offline against real `javac`.
